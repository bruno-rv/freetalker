...
@Suite struct TimelineAndExportTests {
    ...
    @Test func assignsSpeakerWithGreatestTemporalOverlap() {
        let result = joiner.join(
            transcript: [.init(start: 1, end: 5, text: "Hello")],
            speakers: [
                .init(speakerID: "short", start: 1, end: 2),
                .init(speakerID: "long", start: 2, end: 5)
            ]
        )

        #expect(result.map(\.speakerID) == ["long"])
    }

    @Test func overlapTiesResolveDeterministicallyByTurnOrder() {
        ...
        #expect(joiner.join(transcript: transcript, speakers: speakers).first?.speakerID == "first")
        #expect(joiner.join(transcript: transcript, speakers: speakers).first?.speakerID == "first")
    }
    ...
    @Test func exportsAllFourFormatsAndEscapesTheirSyntax() {
        ...
        #expect(exporter.export(segments, format: .markdown, speakerNames: names) == "**A &amp; B:** Use &lt;this&gt; &amp; \u{005C}*that\u{005C}* --&gt; now")
