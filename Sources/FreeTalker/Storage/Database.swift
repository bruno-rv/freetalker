...
final class Database {
    ...
    func statRows() throws -> [DictationStatRow] {
        let stmt = try prepare("SELECT ts, language, template, engine, bundle_id, duration_secs, refined, transcript, requested_output_language FROM dictations;")
        defer { sqlite3_finalize(stmt) }
        ...
        while Self.classifyStep(result) == .row {
            rows.append(DictationStatRow(
                ...
                requestedOutputLanguage: OutputLanguage.persisted(rawValue: columnText(stmt, 8))
            ))
            result = sqlite3_step(stmt)
        }
